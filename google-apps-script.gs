  ...
  MATERIAL_SHEETS.forEach(function(sheetName) {
    ...
    if (sheet) {
      ...
      if (sheetName === 'EPI_PRETO' || sheetName === 'EPI_AMARELO') {
        needed = needed.concat(['item_epi', 'epi_numero', 'tamanho', 'validade_epi']);
...
function processPost(postData) {
  ...
  if (action === 'create_user' || action === 'update_user') {
    if (postData._sessionNivel !== 'ADMIN') {
      return { success: false, message: "Acesso negado: Apenas ADMIN cria usuários" };
    }
    // Se houver senha no payload, realizamos o hashing antes de salvar
    if (postData.senha) {
      var salt = generateSalt();
      postData.salt = salt;
      postData.password_hash = hashPassword(postData.senha, salt);
...
function saveRow(sheetName, data, executor) {
  ...
  for (var key in data) {
    if (key !== 'action' && key !== 'table' && key !== 'executor' && normalizedHeaders.indexOf(key) === -1) {
...
function handleLogin(data) {
  ...
  if (user) {
    ...
    if (user.password_hash && user.salt) {
      ...
      var incomingHash = hashPassword(data.senha, user.salt);
